...
namespace OSAIFileUtility
{
    class CommunicationManager
    {
        ...
        #region WriteData
        public void WriteData(string msg, string strHexAddedToEnd = "")
        {
            switch (CurrentTransmissionType)
            {
                ...
                case TransmissionType.TextWithAddedHex:
                    try
                    {
                        ...
                        strHex += strHexAddedToEnd;
                        //convert the message to byte array
                        byte[] objByteArray = HexToByte(strHex);
                        //send the message to the port
                        comPort.Write(objByteArray, 0, objByteArray.Length);
                        //send the message to the port
                        //display the message
                        DisplayData(MessageType.Outgoing, msg + " + added Hex " + strHexAddedToEnd + "\n");
                    }
                    catch (FormatException ex)
                    {
                        //display error message
